...
//Question: https://www.techiedelight.com/convert-binary-search-tree-into-min-heap/
...
public class IsMinHeap {
    ...
    //To check for the heap property, check that both left and right children are greater than the parent node. This can
    // be easily done while inserting the children into the queue.
    ...
    //The time complexity of the above solution is O(n), where n is the total number of nodes in the binary tree. The
    // auxiliary space required by the program is O(n) for level order traversal, where n is the total number of nodes
    // in the binary tree.
